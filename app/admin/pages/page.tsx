...
export default function PagesListPage() {
  ...
  async function handleSubmitReview(id: string) {
    try {
      const res = await fetch(`/api/pages/${id}/submit-review`, { method: 'POST' })
      if (res.ok) {
        fetchPages()
      } else {
        const data = await res.json()
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-rise-navy">Pages</h1>
        <Link
              ...
              {pages.map((page) => (
                <tr key={page.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/admin/pages/${page.id}/edit`} className="text-rise-navy font-medium hover:underline">
                      {page.title}
                    </Link>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full capitalize ${STATUS_STYLES[page.status] || ''}`}>
